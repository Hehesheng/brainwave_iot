...
uint8_t SDRAM_Test(void)
{
	...
	rt_kprintf("STM32F429 SDRAM Test\r\n");
	rt_kprintf("Test Begin 32Bits Trying...\r\n");	
	for (i = 0; i < SDRAM_SIZE/4; i++)
	{
 		*(__IO uint32_t*) (SDRAM_BANK_ADDR + 4*i) = i;		// 写入数据
	}
	
	rt_kprintf("Write Compelte Then Begin Compare...\r\n");
	for(i = 0; i < SDRAM_SIZE/4;i++ )
	{
		ReadData = *(__IO uint32_t*)(SDRAM_BANK_ADDR + 4 * i );  // 从SDRAM读出数据	
		if( ReadData != i )      //检测数据，若不相等，跳出函数,返回检测失败结果。
		{
			rt_kprintf("SDRAM Test Fail!!\r\n");
			return ERROR;	 // 返回失败标志
		}
	}
	
	rt_kprintf("32Bits Passed try 8bit write\r\n");
	for (i = 0; i < 255; i++)
	{
	...
	for (i = 0; i < 255; i++)
	{
		...
		{
			rt_kprintf("8Bits Test Fail...\r\n");
			rt_kprintf("Check NBL0 and NBL1 Connect\r\n");	
			return ERROR;	 // 返回失败标志
		}
	}		
	rt_kprintf("8Bits Write Passed\r\n");
	rt_kprintf("SDRAM Test OK\r\n");

	return SUCCESS;	 // 返回成功标志
}
MSH_CMD_EXPORT(SDRAM_Test, NONE)
